...
{
    public class DepartmentApproverModels
    {
        //rherejias
        public Dictionary<string, string> GetDepartmentApprovers_cols()
        {
        ...
        public DataTable GetDepartmentApprovers(int offset, int next, string where, string sorting, string searchStr, string userdept)
        {
            ...
            Dictionary<string, string> cols = GetDepartmentApprovers_cols();
            ...
            {
                where = "WHERE IsActive = 1 AND Department = '" + userdept + "'";
            }
            else
            {
                where += " AND IsActive = 1 AND Department = '" + userdept + "'";
            }
         ...
         * @returns     :   datatable
         */
        public int GetDepartmentApprovers_count(string where, string searchStr, string userdept)
        {
            if (where == null || where == "" || where == string.Empty)
            {
                where = "WHERE IsActive = 1 AND Department = '" + userdept + "'";
